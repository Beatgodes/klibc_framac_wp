...
/*@
	...
	behavior eq:
		assumes \forall integer i; 0 <= i < n ==> ToUpper(s1[i]) == ToUpper(s2[i]);
    	ensures \result == 0;

  	behavior not_eq:
  		assumes \exists integer i; 0 <= i < n && ToUpper(s1[i]) != ToUpper(s2[i]);
  		ensures \result != 0;
...
int strncasecmp(const char *s1, const char *s2, size_t n)
{
  ...
  /*@
      ...
      loop invariant \forall integer i; 0 <= i < (\at(n,Pre) - n) ==> ToUpper(s1[i]) == ToUpper(s2[i]);
