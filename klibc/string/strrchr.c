...
char *strrchr(const char *s, int c)
{
  ...
  /*@
		...
		loop invariant found == \null || \base_addr(found) == \base_addr(s);
		loop invariant found == \null || \at(s,Pre) <= found <= \at(s,Pre) + Length(\at(s,Pre));
		loop invariant \at(s,Pre) <= s <= \at(s,Pre) + Length(\at(s,Pre));
		loop invariant (\forall integer i; 0 <= i < s - \at(s,Pre) && \at(s[i],Pre) != (char)c) ==> found == \null;
		loop invariant \exists integer i; 0 <= i < s - \at(s,Pre) && \at(s[i],Pre) == (char)c 
			&& (\forall integer j; i < j < s - \at(s,Pre) && \at(s[i],Pre) != (char)c) ==> found == \at(s,Pre) + i;
		loop invariant found != \null ==>  \forall integer i; found - \at(s, Pre) < i < s - \at(s, Pre) ==> \at(s[i], Pre) != c;
		loop invariant found != \null ==> *found == c;
