...
/*@
	...
	// how to handle mode? O_RDWR | O_CREAT | ...?
	assigns \nothing;
	ensures \result >= 0;
@*/
__extern int open(const char *, int, ...); //used syscals
...
@*/
__extern ssize_t write(int fildes, const void *buf, size_t nbyte); //used syscall
...
/*@
	...
	assigns \nothing;
@*/
__extern int isatty(int fildes); // added var name
